...
useful_val_dirs = ['171204_pose1_sample'] 
...
class CMU_Dataset(Dataset):
    ...
    def __init__(self,cnf,mode='train') -> None:   #cnf --> configuration
        ...
        self.sub_dirs = self.data_path.dirs()  #得到所有根路径下的子文件夹,不同种类的文件夹名称
        self.mode = mode
    ...
    def get_train_data(self):
        ...
        for sub_dir in self.sub_dirs:
            if sub_dir.basename() in useful_train_dirs:  #最终用到的训练集有四种
                ...
                for img_dir in img_dirs:
                    if img_dir.basename() in useful_img_dirs_train:
                        ...
                        anno_idx = random.randint(0,10)
                        anno_count = 1
                        while(anno_idx < len(annotation_files)):
                            if anno_count > 4000:
                                break
                            basename = annotation_files[anno_idx].basename()
                            if basename.endswith('.json'):
                                anno_num = basename.split('.')[0].split('_')[1]
                                img_path = img_dir / (img_dir.basename() +  '_' + anno_num + '.jpg')
                                data_list.append((img_path,annotation_files[anno_idx],cali_file_path,img_dir.basename()))
                            anno_idx += 6
                            anno_count += 1
        return data_list
    ...
    def get_val_data(self):
        """
        using all the images in 16 and 30 dirs of the four different image dirs 
        """
        data_list = []
        for sub_dir in self.sub_dirs:
    ...
    def __getitem__(self, idx):
        ...
        cam_coors , pixel_coors , skel_with_conf= self.reproject(anno_file,cali_file,(lnum,rnum))  #将世界坐标系下的坐标转换到对应相机视角下的相机和像素坐标系中,dedao 
    ...
    def reproject(self,anno_file,cali_file,cam_id):
        ...
        return cam_coors , pixel_coors , skel_with_conf
    ...
    def projectPoints(self,X, K, R, t, Kd):
        ...
        pixel_coors = x  #其实这里的第三行还是在世界坐标系下的
